...
int Cam::grab(unsigned char **frame, uint32_t &bytes_used)
{
    *frame = NULL;
    ...
    // reverse angle for ROBOTIS OP
    unsigned char *pyuv_rev = (unsigned char *)mem[buf.index];
    reverse_yuv(pyuv_rev);
...
// reverse frame for op2(YUV)
void Cam::reverse_yuv(unsigned char *pyuv_reverse)
{
    //    unsigned char *pyuv_original;
    //    int yuv_length = width * height * 2;

    //    pyuv_original = new unsigned char[yuv_length];
    //    memcpy(pyuv_original, pyuv_reverse, yuv_length);

    //    for(int ix = 0; ix < yuv_length; ix += 4)
    //    {
    //        pyuv_reverse[ix]    = pyuv_original[yuv_length - ix-2];
    //        pyuv_reverse[ix+1]  = pyuv_original[yuv_length - ix-3];
    //        pyuv_reverse[ix+2]  = pyuv_original[yuv_length - ix-4];
    //        pyuv_reverse[ix+3]  = pyuv_original[yuv_length - ix-1];
    //    }

    //    delete[] pyuv_original;

    int yuv_length = width * height * 2;

    int _y1, _u, _y2, _v;
    for(int ix = 0; ix < yuv_length * 0.5; ix += 4)
    {
        // store
        _y1 = pyuv_reverse[ix];
        _u  = pyuv_reverse[ix+1];
        _y2 = pyuv_reverse[ix+2];
        _v  = pyuv_reverse[ix+3];

        pyuv_reverse[ix]    = pyuv_reverse[yuv_length - ix-2]; // y2 -> y1
        pyuv_reverse[ix+1]  = pyuv_reverse[yuv_length - ix-3];
        pyuv_reverse[ix+2]  = pyuv_reverse[yuv_length - ix-4]; // y1 -> y2
        pyuv_reverse[ix+3]  = pyuv_reverse[yuv_length - ix-1];

        pyuv_reverse[yuv_length - ix-4] = _y2; // y2 -> y1
        pyuv_reverse[yuv_length - ix-3] = _u;
        pyuv_reverse[yuv_length - ix-2] = _y1; // y1 -> y2
        pyuv_reverse[yuv_length - ix-1] = _v;
    }
}

// reverse frame for op2(RGB)
void Cam::reverse_rgb(unsigned char *prgb_reverse)
{
    unsigned char *prgb_original;
    int rgb_length = width * height * 3;

    prgb_original = new unsigned char[rgb_length];
    memcpy(prgb_original, prgb_reverse, rgb_length);

    for(int ix = 0; ix < rgb_length; ix += 3)
    {
        prgb_reverse[ix]     = prgb_original[rgb_length - ix-3];
        prgb_reverse[ix+1]   = prgb_original[rgb_length - ix-2];
        prgb_reverse[ix+2]   = prgb_original[rgb_length - ix-1];
    }

    delete[] prgb_original;
}
